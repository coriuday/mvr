...
impl UniversityService {
    ...
    pub async fn list(&self, filter: &UniversityFilter) -> AppResult<(Vec<University>, i64)> {
        UniversityRepository::new(self.db.clone()).find_all(filter).await
    }
    ...
    pub async fn create(&self, body: &CreateUniversityRequest) -> AppResult<University> {
        if body.name.trim().is_empty() {
            return Err(AppError::BadRequest("University name is required".to_string()));
        }
        UniversityRepository::new(self.db.clone()).create(body).await
    }

    /// Update the featured flag of a university (admin).
    pub async fn update_featured(&self, id: Uuid, body: &UpdateUniversityRequest) -> AppResult<University> {
        let featured = body.is_featured.unwrap_or(false);
        UniversityRepository::new(self.db.clone()).update_featured(id, featured).await
    }
