...
fn build_prompt(req: &SopReviewRequest) -> String {
    let country_ctx = req.country.as_deref().unwrap_or("any English-speaking country");
    let degree_ctx  = req.degree.as_ref().map(DegreeLevel::label).unwrap_or("postgraduate");
    ...
    pub async fn review_sop(&self, req: &SopReviewRequest) -> AppResult<SopReviewResult> {
        if !self.is_configured() {
            return Err(AppError::InternalServerError(
                "AI service is not configured on this server. Please add GEMINI_API_KEY.".to_string(),
            ));
        }
        ...
        if word_count > 3000 {
            return Err(AppError::BadRequest(
                "Please submit a maximum of 3,000 words. Split into sections if needed.".to_string(),
            ));
        }
        ...
        tracing::info!(
            ...
            sop.degree     = req.degree.as_ref().map(DegreeLevel::label).unwrap_or("unset"),
        ...
        let tokens_used = gemini_resp.usage_metadata
            .as_ref()
            .and_then(|u| u.total_token_count);

        let raw_json = gemini_resp.candidates
            .into_iter()
            .next()
        ...
        let mut result: SopReviewResult = serde_json::from_str(&raw_json)
            .map_err(|e| {
                tracing::error!(
                    sop.word_count = word_count,
                    parse_error    = %e,
        ...
        tracing::info!(
            ...
            gemini.est_cost_usd  = tokens_used.map(|t| format!("{:.5}", t as f64 * 0.0000003))
